...
static unsigned TestcaseIdx = 0;    //его тоже нужно из файла менять как то, а может и нет
...
static const unsigned TestcaseCount = 34;                    //ИЗМЕНИЛ КОЛ-ВО ТЕСТОВ 33 -> 34
...
static int LabTimeout = 3000;      //ВРЕМЯ НА РАБОТУ 
...
static TTestcaseData GetFromTestcase(unsigned testcaseIdx, enum ETestcaseDataId dataId, unsigned edgeIdx) {
    ...
    if (testcaseIdx < smallTestCount) {
        ...
        static const TSmallTest smallTests[] = {
            ...
            {MAX_VERTEX_COUNT1 + 1, 1, {{1, 1, 1}}, "bad number of vertices"},
