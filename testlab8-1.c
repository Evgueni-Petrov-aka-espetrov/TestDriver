...
int SpecialFeed(void) {
    ...
    const unsigned vertexCount = VERTEX_FOR_PRIM + 1;
    const unsigned edgeCount = 1;
...
int SpecialFeed2()
{
    ...
    printf("done in T=%u seconds. Starting exe with timeout 2*T+3 seconds... ", start / 1000);
    LabTimeout = 2*(int)start + 3000;
